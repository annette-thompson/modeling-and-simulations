function dy = Combined_Pathway_Model_Thiol(t,y,param_list,opt_struct)
...
%TesA
ket_str = opt_struct.Ketone_params;
ind = ket_str.count;
thio_scaling = [0.454318854716137	2.19560691520507	68.4348965082940	12.9343678377699];
if strcmp(ket_str.ket_cond.TE_ident(ind),'TesA')
    k7_1f = param_list.k7_1f;
elseif strcmp(ket_str.ket_cond.TE_ident(ind),'CpFatB')
    k7_1f = thio_scaling(3).*param_list.k7_1f;
elseif strcmp(ket_str.ket_cond.TE_ident(ind),'BTE')
    k7_1f = thio_scaling(4).*param_list.k7_1f;
elseif strcmp(ket_str.ket_cond.TE_ident(ind),'chFatB2')
    k7_1f = param_list.k7_1f;
else
    k7_1f = 0;
end
...
%Beta-oxidation and Ketone constants
if strcmp(ket_str.ket_cond.FadD_ident(ind),'ecFadD')
    ...
    kcatD1 = opt_struct.Ketone_vars(1).*0.029.*[0.103 0.132 0.299 1.000 0.798 0.704 0.434 1.000 0.798 0.704 0.519]; %10.1042/0264-6021:3600699 AND Promiscuous Fatty Acyl-CoA Ligases Produce Acyl-CoA and Acyl-SNAC Precursors for Polyketide Biosynthesis
    kcatD2 = opt_struct.Ketone_vars(2).*10*0.029; %10.1042/0264-6021:3600699
elseif strcmp(ket_str.ket_cond.FadD_ident(ind),'FadD6')
    Km_ATP = 36; %doi.org/10.1016/0167-4838(85)90269-9
    Km_FA_AMP = [11.2  40.8  4.3 1.6  2.4  2.4  8.9  1.6  2.4  2.4  8.5]; %doi.org/10.1016/S0021-9258(19)69262-8
    Km_FA = [11.2  40.8  4.3 1.6  2.4  2.4  8.9  1.6  2.4  2.4  8.5]; %doi.org/10.1016/S0021-9258(19)69262-8
    Km_CoA = 32.99; %Promiscuous Fatty Acyl-CoA Ligases Produce Acyl-CoA and Acyl-SNAC Precursors for Polyketide Biosynthesis
    kcatD1 = opt_struct.Ketone_vars(1).*0.01967.*[0.103 0.132 0.299 1.000 0.798 0.704 0.434 1.000 0.798 0.704 0.519]; %10.1042/0264-6021:3600699 AND Promiscuous Fatty Acyl-CoA Ligases Produce Acyl-CoA and Acyl-SNAC Precursors for Polyketide Biosynthesis
    kcatD2 = opt_struct.Ketone_vars(2).*10.*0.01967; %10.1042/0264-6021:3600699 
end

%FadE
if strcmp(ket_str.ket_cond.FadE_ident(ind),'ecFadE')
    Km_FAD = 1.9; %doi.org/10.1016/S0021-9258(18)33160-0
    Km_FA_CoA = 0.947;%doi.org/10.1016/S0021-9258(18)33160-0
    kcatE = opt_struct.Ketone_vars(3)*19.03; %doi.org/10.1016/S0021-9258(20)71246-9
elseif strcmp(ket_str.ket_cond.FadE_ident(ind),'ACO')
    Km_FAD = 1.9; %doi.org/10.1016/S0021-9258(18)33160-0
    Km_FA_CoA = 0.947;%doi.org/10.1016/S0021-9258(18)33160-0
    kcatE = opt_struct.Ketone_vars(3)*10*19.03; %doi.org/10.1016/S0021-9258(20)71246-9
end

%FadB
% All parameters from Biochemistry 1996, 35, 9625-9630
if strcmp(ket_str.ket_cond.FadB_ident(ind),'ecFadB')
    Km_FA_enoyl_CoA = 53;
    kcatB1 = 775;
    Km_NAD = 2;
    Km_hyd_acyl_CoA = 69; 
    kcatB2 = 382;
elseif strcmp(ket_str.ket_cond.FadB_ident(ind),'PpFadB')
    Km_FA_enoyl_CoA = 53;
    kcatB1 = 2*775;
    Km_NAD = 2; 
    Km_hyd_acyl_CoA = 69; 
...
%FadM 
%10.1021/bi800595f
if strcmp(ket_str.ket_cond.FadM_ident(ind),'ecFadM')
    kcatM = opt_struct.Ketone_vars(4).*[0 1.6 1.6 1.6 6.2 10.2 0 21.6 23.7 10.2 0];
    Km_FadM_keto_acyl_CoA = [440 13.0 13.0 13.0 6.3 6.0 440 10.8 3.0 6.0 440];
    kcatM_ACP = 0.*kcatM;
elseif strcmp(ket_str.ket_cond.FadM_ident(ind),'PsFadM')
    kcatM = opt_struct.Ketone_vars(4).*1.5*[0 1.6 1.6 1.6 6.2 10.2 0 21.6 23.7 10.2 0];
    Km_FadM_keto_acyl_CoA = [440 13.0 13.0 13.0 6.3 6.0 440 10.8 3.0 6.0 440];
    kcatM_ACP = 0.*kcatM;
end

%FadA
% all parameters from J Biol Chem. 1993 Mar 25;268(9):6588-92.
if strcmp(ket_str.ket_cond.FadA_ident(ind),'ecFadA')
    kcatfadA = opt_struct.opt_vec.*104;
elseif strcmp(ket_str.ket_cond.FadA_ident(ind),'BktB')
    kcatfadA = opt_struct.opt_vec.*5.393939394*104;
end
Km_CoA_A = 102;
Km_acCoA = 96;
Km_FA_CoA_A = 96;
...
FadD = ket_str.ket_cond.FadD_conc(ind);
FadE = ket_str.ket_cond.FadE_conc(ind);
FadB = ket_str.ket_cond.FadB_conc(ind);
FadM = ket_str.ket_cond.FadM_conc(ind);
FadA = ket_str.ket_cond.FadA_conc(ind);


%TesA activity on acyl-CoAs
CoA_ACP_Km_Kd_ratio = 1.*(1/54);%Estimated ratio of KcatCoA/KcatACP for C14
if strcmp(ket_str.ket_cond.TE_ident(ind),'TesA')
    kcatTesA_CoA = [param_list.kcat7(2:8) param_list.kcat7(5) param_list.kcat7(6) param_list.kcat7(7) param_list.kcat7(8)];
    Km_TesA_FA_CoA = [4815.2 434 48 50 107 97 812 50 107 53 81];
elseif strcmp(ket_str.ket_cond.TE_ident(ind),'CpFatB')
    kcatTesA_CoA = [param_list.kcat7(2:8) param_list.kcat7(5) param_list.kcat7(6) param_list.kcat7(7) param_list.kcat7(8)];
    kd_est = param_list.k7_1r./(param_list.k7_1f);
    Km_TesA_FA_CoA = CoA_ACP_Km_Kd_ratio*[kd_est(2:8) kd_est(5) kd_est(6) kd_est(7) kd_est(8)];
elseif strcmp(ket_str.ket_cond.TE_ident(ind),'BTE')
    kcatTesA_CoA = [param_list.kcat7(2:8) param_list.kcat7(5) param_list.kcat7(6) param_list.kcat7(7) param_list.kcat7(8)];
    kd_est = param_list.k7_1r./(param_list.k7_1f);
    Km_TesA_FA_CoA = CoA_ACP_Km_Kd_ratio*[kd_est(2:8) kd_est(5) kd_est(6) kd_est(7) kd_est(8)];
elseif strcmp(ket_str.ket_cond.TE_ident(ind),'chFatB2')
    ...
    kd_est = param_list.k7_1r./(param_list.k7_1f);
    Km_TesA_FA_CoA = CoA_ACP_Km_Kd_ratio*[kd_est(2:8) kd_est(5) kd_est(6) kd_est(7) kd_est(8)];
elseif strcmp(ket_str.ket_cond.TE_ident(ind),'atFatA3')
    kcatTesA_CoA = [param_list.kcat7(2:8) param_list.kcat7(5) param_list.kcat7(6) param_list.kcat7(7) param_list.kcat7(8)];
    kd_est = param_list.k7_1r./(param_list.k7_1f);
    Km_TesA_FA_CoA = CoA_ACP_Km_Kd_ratio*[kd_est(2:8) kd_est(5) kd_est(6) kd_est(7) kd_est(8)];
end
...
%TesA on Acyl-CoA parameters (raw values)
% kcatTesA_CoA = [51.73 39 11.9 131 157 156 331 131 157 62 27];
% Km_TesA_FA_CoA = [4815.2 434 48 50 107 97 812 50 107 53 81];
...
dy(3,:) = k1_3r.*y(86,:) + k3_1r.*y(90,:) - k1_3f.*y(85,:).*y(3,:) - k3_1f.*e3.*y(3,:)...
    ...
    - kcatfadA.*FadA.*y(3,:).*y(348,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(348,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(382,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(382,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(349,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(349,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(383,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(383,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(350,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(350,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(384,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(384,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(351,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(351,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(385,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(385,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(352,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(352,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(386,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(386,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(353,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(353,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(387,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(387,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(354,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(354,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(392,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(392,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(355,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(355,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(389,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(389,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(356,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(356,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(390,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(390,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(357,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(357,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(391,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(391,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(358,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(358,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(393,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(393,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(348,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(348,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(382,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(382,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(349,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(349,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(383,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(383,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(350,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(350,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(384,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(385,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(351,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(351,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(385,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(385,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(352,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(352,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(386,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(386,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(353,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(353,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(387,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(387,:)))...
    - kcatfadA.*FadA.*y(3,:).*y(355,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(355,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(389,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(389,:)))...
...
dy(4,:) = k2_3r.*y(89,:) - k2_3f.*y(88,:).*y(4,:)...
    ...
    +kcatM_ACP(1)*FadM*y(17,:)./(Km_FadM_keto_acyl_CoA(1)+y(17,:))...
    +kcatM_ACP(2)*FadM*y(22,:)./(Km_FadM_keto_acyl_CoA(2)+y(22,:))...
    +kcatM_ACP(3)*FadM*y(27,:)./(Km_FadM_keto_acyl_CoA(3)+y(27,:))...
    +kcatM_ACP(4)*FadM*y(33,:)./(Km_FadM_keto_acyl_CoA(4)+y(33,:))...
    +kcatM_ACP(5)*FadM*y(43,:)./(Km_FadM_keto_acyl_CoA(5)+y(43,:))...
    +kcatM_ACP(6)*FadM*y(53,:)./(Km_FadM_keto_acyl_CoA(6)+y(53,:))...
    +kcatM_ACP(7)*FadM*y(63,:)./(Km_FadM_keto_acyl_CoA(7)+y(63,:))...
    +kcatM_ACP(8)*FadM*y(38,:)./(Km_FadM_keto_acyl_CoA(8)+y(38,:))...
    +kcatM_ACP(9)*FadM*y(48,:)./(Km_FadM_keto_acyl_CoA(9)+y(48,:))...
    +kcatM_ACP(10)*FadM*y(58,:)./(Km_FadM_keto_acyl_CoA(10)+y(58,:))...
    +kcatM_ACP(11)*FadM*y(68,:)./(Km_FadM_keto_acyl_CoA(11)+y(68,:))...
    ;
...
dy(17,:) = kcat8(1).*y(104,:)+ kcat10(1).*y(109,:) + k4_2r(2).*y(112,:)- k4_2f(2).*y(93,:).*y(17,:) - kcatM_ACP(1)*FadM*y(17,:)./(Km_FadM_keto_acyl_CoA(1)+y(17,:));
dy(22,:) = kcat8(2).*y(119,:)+ kcat10(2).*y(124,:) + k4_2r(3).*y(127,:)- k4_2f(3).*y(93,:).*y(22,:) - kcatM_ACP(2)*FadM*y(22,:)./(Km_FadM_keto_acyl_CoA(2)+y(22,:));
dy(27,:) = kcat8(3).*y(134,:)+ kcat10(3).*y(139,:) + k4_2r(4).*y(142,:)- k4_2f(4).*y(93,:).*y(27,:) - kcatM_ACP(3)*FadM*y(27,:)./(Km_FadM_keto_acyl_CoA(3)+y(27,:));
dy(33,:) = kcat8(4).*y(149,:)+ kcat10(4).*y(154,:) + k4_2r(5).*y(157,:)- k4_2f(5).*y(93,:).*y(33,:) - kcatM_ACP(4)*FadM*y(33,:)./(Km_FadM_keto_acyl_CoA(4)+y(33,:));
dy(43,:) = kcat8(5).*y(164,:)+ kcat10(5).*y(169,:) + k4_2r(6).*y(172,:)- k4_2f(6).*y(93,:).*y(43,:) - kcatM_ACP(5)*FadM*y(43,:)./(Km_FadM_keto_acyl_CoA(5)+y(43,:));
dy(53,:) = kcat8(6).*y(179,:)+ kcat10(6).*y(184,:) + k4_2r(7).*y(187,:)- k4_2f(7).*y(93,:).*y(53,:) - kcatM_ACP(6)*FadM*y(53,:)./(Km_FadM_keto_acyl_CoA(6)+y(53,:));
dy(63,:) = kcat8(7).*y(194,:)+ kcat10(7).*y(199,:) + k4_2r(8).*y(202,:)- k4_2f(8).*y(93,:).*y(63,:) - kcatM_ACP(7)*FadM*y(63,:)./(Km_FadM_keto_acyl_CoA(7)+y(63,:));
dy(73,:) = kcat8(8).*y(209,:)+ kcat10(8).*y(214,:) + k4_2r(9).*y(217,:)- k4_2f(9).*y(93,:).*y(73,:);

dy(38,:) = kcat8_un(4).*y(308,:) + kcat10_un(4).*y(304,:) + k4_2r(5).*y(226,:)- k4_2f(5).*y(93,:).*y(38,:) - kcatM_ACP(8)*FadM*y(38,:)./(Km_FadM_keto_acyl_CoA(8)+y(38,:));
dy(48,:) = kcat8_un(5).*y(233,:) + kcat10_un(5).*y(238,:) + k4_2r(6).*y(241,:)- k4_2f(6).*y(93,:).*y(48,:) - kcatM_ACP(9)*FadM*y(48,:)./(Km_FadM_keto_acyl_CoA(9)+y(48,:));
dy(58,:) = kcat8_un(6).*y(248,:) + kcat10_un(6).*y(253,:) + k4_2r(7).*y(256,:)- k4_2f(7).*y(93,:).*y(58,:) - kcatM_ACP(10)*FadM*y(58,:)./(Km_FadM_keto_acyl_CoA(10)+y(58,:));
dy(68,:) = kcat8_un(7).*y(263,:) + kcat10_un(7).*y(268,:) + k4_2r(8).*y(271,:)- k4_2f(8).*y(93,:).*y(68,:) - kcatM_ACP(11)*FadM*y(68,:)./(Km_FadM_keto_acyl_CoA(11)+y(68,:));
...
%FFA Production. 
dy(16,:) = kcat7(1).*y(101,:);
dy(21,:) = kcat7(2).*y(116,:) - kcatD1(1).*FadD.*y(1,:).*y(21,:)./((Km_ATP+y(1,:)).*(Km_FA(1)+y(21,:))) + kcatTesA_CoA(1).*e7.*y(348,:)./(Km_TesA_FA_CoA(1)+y(348,:));
dy(26,:) = kcat7(3).*y(131,:) - kcatD1(2).*FadD.*y(1,:).*y(26,:)./((Km_ATP+y(1,:)).*(Km_FA(2)+y(26,:))) + kcatTesA_CoA(2).*e7.*y(349,:)./(Km_TesA_FA_CoA(2)+y(349,:));
dy(31,:) = kcat7(4).*y(146,:) - kcatD1(3).*FadD.*y(1,:).*y(31,:)./((Km_ATP+y(1,:)).*(Km_FA(3)+y(31,:))) + kcatTesA_CoA(3).*e7.*y(350,:)./(Km_TesA_FA_CoA(3)+y(350,:));
dy(37,:) = kcat7(5).*y(161,:) - kcatD1(4).*FadD.*y(1,:).*y(37,:)./((Km_ATP+y(1,:)).*(Km_FA(4)+y(37,:))) + kcatTesA_CoA(4).*e7.*y(351,:)./(Km_TesA_FA_CoA(4)+y(351,:));
dy(47,:) = kcat7(6).*y(176,:) - kcatD1(5).*FadD.*y(1,:).*y(47,:)./((Km_ATP+y(1,:)).*(Km_FA(5)+y(47,:))) + kcatTesA_CoA(5).*e7.*y(352,:)./(Km_TesA_FA_CoA(5)+y(352,:));
dy(57,:) = kcat7(7).*y(191,:) - kcatD1(6).*FadD.*y(1,:).*y(57,:)./((Km_ATP+y(1,:)).*(Km_FA(6)+y(57,:))) + kcatTesA_CoA(6).*e7.*y(353,:)./(Km_TesA_FA_CoA(6)+y(353,:));
dy(67,:) = kcat7(8).*y(206,:) - kcatD1(7).*FadD.*y(1,:).*y(67,:)./((Km_ATP+y(1,:)).*(Km_FA(7)+y(67,:))) + kcatTesA_CoA(7).*e7.*y(354,:)./(Km_TesA_FA_CoA(7)+y(354,:));
dy(77,:) = kcat7(9).*y(221,:);

dy(42,:) = kcat7(5).*y(230,:) - kcatD1(8).*FadD.*y(1,:).*y(42,:)./((Km_ATP+y(1,:)).*(Km_FA(8)+y(42,:))) + kcatTesA_CoA(8).*e7.*y(355,:)./(Km_TesA_FA_CoA(8)+y(355,:));
dy(52,:) = kcat7(6).*y(245,:) - kcatD1(9).*FadD.*y(1,:).*y(52,:)./((Km_ATP+y(1,:)).*(Km_FA(9)+y(52,:))) + kcatTesA_CoA(9).*e7.*y(356,:)./(Km_TesA_FA_CoA(9)+y(356,:));
dy(62,:) = kcat7(7).*y(260,:) - kcatD1(10).*FadD.*y(1,:).*y(62,:)./((Km_ATP+y(1,:)).*(Km_FA(10)+y(62,:))) + kcatTesA_CoA(10).*e7.*y(357,:)./(Km_TesA_FA_CoA(10)+y(357,:));
dy(72,:) = kcat7(8).*y(275,:) - kcatD1(11).*FadD.*y(1,:).*y(72,:)./((Km_ATP+y(1,:)).*(Km_FA(11)+y(72,:))) + kcatTesA_CoA(11).*e7.*y(358,:)./(Km_TesA_FA_CoA(11)+y(358,:));
dy(82,:) = kcat7(9).*y(290,:);
...
%Beta-oxidation pathway, and FadM
%FadD Rxn 1 (rate of formation of acyl-AMP intermediate)
...
dy(348,:) = kcatD2.*FadD.*y(9,:).*y(337,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(1)+y(337,:))) - kcatE.*FadE.*FAD.*y(348,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(348,:))) - kcatTesA_CoA(1).*e7.*y(348,:)./(Km_TesA_FA_CoA(1)+y(348,:))...
        - kcatfadA.*FadA.*y(3,:).*y(348,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(348,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(382,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(382,:)));%6
dy(349,:) = kcatD2.*FadD.*y(9,:).*y(338,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(2)+y(338,:))) - kcatE.*FadE.*FAD.*y(349,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(349,:))) - kcatTesA_CoA(2).*e7.*y(349,:)./(Km_TesA_FA_CoA(2)+y(349,:))...
        - kcatfadA.*FadA.*y(3,:).*y(349,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(349,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(383,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(383,:)));%8
dy(350,:) = kcatD2.*FadD.*y(9,:).*y(339,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(3)+y(339,:))) - kcatE.*FadE.*FAD.*y(350,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(350,:))) - kcatTesA_CoA(3).*e7.*y(350,:)./(Km_TesA_FA_CoA(3)+y(350,:))...
        - kcatfadA.*FadA.*y(3,:).*y(350,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(350,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(384,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(384,:)));%10
dy(351,:) = kcatD2.*FadD.*y(9,:).*y(340,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(4)+y(340,:))) - kcatE.*FadE.*FAD.*y(351,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(351,:))) - kcatTesA_CoA(4).*e7.*y(351,:)./(Km_TesA_FA_CoA(4)+y(351,:))...
        - kcatfadA.*FadA.*y(3,:).*y(351,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(351,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(385,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(385,:)));%12
dy(352,:) = kcatD2.*FadD.*y(9,:).*y(341,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(5)+y(341,:))) - kcatE.*FadE.*FAD.*y(352,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(352,:))) - kcatTesA_CoA(5).*e7.*y(352,:)./(Km_TesA_FA_CoA(5)+y(352,:))...
        - kcatfadA.*FadA.*y(3,:).*y(352,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(352,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(386,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(386,:)));%14
dy(353,:) = kcatD2.*FadD.*y(9,:).*y(342,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(6)+y(342,:))) - kcatE.*FadE.*FAD.*y(353,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(353,:))) - kcatTesA_CoA(6).*e7.*y(353,:)./(Km_TesA_FA_CoA(6)+y(353,:))...
        - kcatfadA.*FadA.*y(3,:).*y(353,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(353,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(387,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(387,:)));%16
dy(354,:) = kcatD2.*FadD.*y(9,:).*y(343,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(7)+y(343,:))) - kcatE.*FadE.*FAD.*y(354,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(354,:))) - kcatTesA_CoA(7).*e7.*y(354,:)./(Km_TesA_FA_CoA(7)+y(354,:))...
        - kcatfadA.*FadA.*y(3,:).*y(354,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(354,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(392,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(392,:)));%18
dy(355,:) = kcatD2.*FadD.*y(9,:).*y(344,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(8)+y(344,:))) - kcatE.*FadE.*FAD.*y(355,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(355,:))) - kcatTesA_CoA(8).*e7.*y(355,:)./(Km_TesA_FA_CoA(8)+y(355,:))...
        - kcatfadA.*FadA.*y(3,:).*y(355,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(355,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(389,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(389,:)));%12_unsat
dy(356,:) = kcatD2.*FadD.*y(9,:).*y(345,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(9)+y(345,:))) - kcatE.*FadE.*FAD.*y(356,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(356,:))) - kcatTesA_CoA(9).*e7.*y(356,:)./(Km_TesA_FA_CoA(9)+y(356,:))...
        - kcatfadA.*FadA.*y(3,:).*y(356,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(356,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(390,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(390,:)));%14_unsat
dy(357,:) = kcatD2.*FadD.*y(9,:).*y(346,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(10)+y(346,:))) - kcatE.*FadE.*FAD.*y(357,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(357,:))) - kcatTesA_CoA(10).*e7.*y(357,:)./(Km_TesA_FA_CoA(10)+y(357,:))...
        - kcatfadA.*FadA.*y(3,:).*y(357,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(357,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(391,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(391,:)));%16_unsat
dy(358,:) = kcatD2.*FadD.*y(9,:).*y(347,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(11)+y(347,:))) - kcatE.*FadE.*FAD.*y(358,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(358,:))) - kcatTesA_CoA(11).*e7.*y(358,:)./(Km_TesA_FA_CoA(11)+y(358,:))...
        - kcatfadA.*FadA.*y(3,:).*y(358,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(358,:))) + kcatfadA_rev.*FadA.*y(9,:).*y(393,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(393,:)));%18_unsat

%FadE rate of formation of FA-enoyl-CoA
dy(359,:) = kcatE.*FadE.*FAD.*y(348,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(348,:))) - kcatB1.*FadB.*y(359,:)./(Km_FA_enoyl_CoA+y(359,:));%6
dy(360,:) = kcatE.*FadE.*FAD.*y(349,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(349,:))) - kcatB1.*FadB.*y(360,:)./(Km_FA_enoyl_CoA+y(360,:));%8
dy(361,:) = kcatE.*FadE.*FAD.*y(350,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(350,:))) - kcatB1.*FadB.*y(361,:)./(Km_FA_enoyl_CoA+y(361,:));%10
dy(362,:) = kcatE.*FadE.*FAD.*y(351,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(351,:))) - kcatB1.*FadB.*y(362,:)./(Km_FA_enoyl_CoA+y(362,:));%12
dy(363,:) = kcatE.*FadE.*FAD.*y(352,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(352,:))) - kcatB1.*FadB.*y(363,:)./(Km_FA_enoyl_CoA+y(363,:));%14
dy(364,:) = kcatE.*FadE.*FAD.*y(353,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(353,:))) - kcatB1.*FadB.*y(364,:)./(Km_FA_enoyl_CoA+y(364,:));%16
dy(365,:) = kcatE.*FadE.*FAD.*y(354,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(354,:))) - kcatB1.*FadB.*y(365,:)./(Km_FA_enoyl_CoA+y(365,:));%18
dy(366,:) = kcatE.*FadE.*FAD.*y(355,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(355,:))) - kcatB1.*FadB.*y(366,:)./(Km_FA_enoyl_CoA+y(366,:));%12_unsat
dy(367,:) = kcatE.*FadE.*FAD.*y(356,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(356,:))) - kcatB1.*FadB.*y(367,:)./(Km_FA_enoyl_CoA+y(367,:));%14_unsat
dy(368,:) = kcatE.*FadE.*FAD.*y(357,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(357,:))) - kcatB1.*FadB.*y(368,:)./(Km_FA_enoyl_CoA+y(368,:));%16_unsat
dy(369,:) = kcatE.*FadE.*FAD.*y(358,:)./((Km_FAD+FAD).*(Km_FA_CoA+y(358,:))) - kcatB1.*FadB.*y(369,:)./(Km_FA_enoyl_CoA+y(369,:));%18_unsat

%FadB Rxn 1 rate of formation of FA_hydroxy-acyl-CoA
dy(370,:) = kcatB1.*FadB.*y(359,:)./(Km_FA_enoyl_CoA+y(359,:)) - kcatB2.*FadB.*NAD.*y(370,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(370,:)));%6
dy(371,:) = kcatB1.*FadB.*y(360,:)./(Km_FA_enoyl_CoA+y(360,:)) - kcatB2.*FadB.*NAD.*y(371,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(371,:)));%8
dy(372,:) = kcatB1.*FadB.*y(361,:)./(Km_FA_enoyl_CoA+y(361,:)) - kcatB2.*FadB.*NAD.*y(372,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(372,:)));%10
dy(373,:) = kcatB1.*FadB.*y(362,:)./(Km_FA_enoyl_CoA+y(362,:)) - kcatB2.*FadB.*NAD.*y(373,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(373,:)));%12
dy(374,:) = kcatB1.*FadB.*y(363,:)./(Km_FA_enoyl_CoA+y(363,:)) - kcatB2.*FadB.*NAD.*y(374,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(374,:)));%14
dy(375,:) = kcatB1.*FadB.*y(364,:)./(Km_FA_enoyl_CoA+y(364,:)) - kcatB2.*FadB.*NAD.*y(375,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(375,:)));%16
dy(376,:) = kcatB1.*FadB.*y(365,:)./(Km_FA_enoyl_CoA+y(365,:)) - kcatB2.*FadB.*NAD.*y(376,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(376,:)));%18
dy(377,:) = kcatB1.*FadB.*y(366,:)./(Km_FA_enoyl_CoA+y(366,:)) - kcatB2.*FadB.*NAD.*y(377,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(377,:)));%12_unsat
dy(378,:) = kcatB1.*FadB.*y(367,:)./(Km_FA_enoyl_CoA+y(367,:)) - kcatB2.*FadB.*NAD.*y(378,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(378,:)));%14_unsat
dy(379,:) = kcatB1.*FadB.*y(368,:)./(Km_FA_enoyl_CoA+y(368,:)) - kcatB2.*FadB.*NAD.*y(379,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(379,:)));%16_unsat
dy(380,:) = kcatB1.*FadB.*y(369,:)./(Km_FA_enoyl_CoA+y(369,:)) - kcatB2.*FadB.*NAD.*y(380,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(380,:)));%18_unsat

%FadB Rxn 2 rate of formation of FA_keto-acyl-CoA
dy(381,:) = kcatB2.*FadB.*NAD.*y(370,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(370,:))) - kcatM(1).*FadM.*y(381,:)./(Km_FadM_keto_acyl_CoA(1)+y(381,:));%6
dy(382,:) = kcatB2.*FadB.*NAD.*y(371,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(371,:))) - kcatM(2).*FadM.*y(382,:)./(Km_FadM_keto_acyl_CoA(2)+y(382,:))...
    +kcatfadA.*FadA.*y(3,:).*y(348,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(348,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(382,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(382,:)));%8
dy(383,:) = kcatB2.*FadB.*NAD.*y(372,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(372,:))) - kcatM(3).*FadM.*y(383,:)./(Km_FadM_keto_acyl_CoA(3)+y(383,:))...
    +kcatfadA.*FadA.*y(3,:).*y(349,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(349,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(383,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(383,:)));%10
dy(384,:) = kcatB2.*FadB.*NAD.*y(373,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(373,:))) - kcatM(4).*FadM.*y(384,:)./(Km_FadM_keto_acyl_CoA(4)+y(384,:))...
    +kcatfadA.*FadA.*y(3,:).*y(350,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(350,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(384,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(385,:)));%12
dy(385,:) = kcatB2.*FadB.*NAD.*y(374,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(374,:))) - kcatM(5).*FadM.*y(385,:)./(Km_FadM_keto_acyl_CoA(5)+y(385,:))...
    +kcatfadA.*FadA.*y(3,:).*y(351,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(351,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(385,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(385,:)));%14
dy(386,:) = kcatB2.*FadB.*NAD.*y(375,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(375,:))) - kcatM(6).*FadM.*y(386,:)./(Km_FadM_keto_acyl_CoA(6)+y(386,:))...
    +kcatfadA.*FadA.*y(3,:).*y(352,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(352,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(386,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(386,:)));%16
dy(387,:) = kcatB2.*FadB.*NAD.*y(376,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(376,:))) - kcatM(7).*FadM.*y(387,:)./(Km_FadM_keto_acyl_CoA(7)+y(387,:))...
    +kcatfadA.*FadA.*y(3,:).*y(353,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(353,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(387,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(387,:)));%18
dy(388,:) = kcatB2.*FadB.*NAD.*y(377,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(377,:))) - kcatM(8).*FadM.*y(388,:)./(Km_FadM_keto_acyl_CoA(8)+y(388,:));%12_unsat
dy(389,:) = kcatB2.*FadB.*NAD.*y(378,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(378,:))) - kcatM(9).*FadM.*y(389,:)./(Km_FadM_keto_acyl_CoA(9)+y(389,:))...
    +kcatfadA.*FadA.*y(3,:).*y(355,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(355,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(389,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(389,:)));%14_unsat
dy(390,:) = kcatB2.*FadB.*NAD.*y(379,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(379,:))) - kcatM(10).*FadM.*y(390,:)./(Km_FadM_keto_acyl_CoA(10)+y(390,:))...
    +kcatfadA.*FadA.*y(3,:).*y(356,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(356,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(390,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(390,:)));%16_unsat
dy(391,:) = kcatB2.*FadB.*NAD.*y(380,:)./((Km_NAD+NAD).*(Km_hyd_acyl_CoA+y(380,:))) - kcatM(11).*FadM.*y(391,:)./(Km_FadM_keto_acyl_CoA(11)+y(391,:))...
    +kcatfadA.*FadA.*y(3,:).*y(357,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(357,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(391,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(391,:)));%18_unsat
dy(392,:) = kcatfadA.*FadA.*y(3,:).*y(354,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(354,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(392,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(392,:))) - kcatM(7).*FadM.*y(392,:)./(Km_FadM_keto_acyl_CoA(7)+y(392,:));%20
dy(393,:) = kcatfadA.*FadA.*y(3,:).*y(358,:)./((Km_acCoA+y(3,:)).*(Km_FA_CoA_A+y(358,:))) - kcatfadA_rev.*FadA.*y(9,:).*y(393,:)./((Km_CoA_A+y(9,:)).*(Km_FA_CoA_A+y(393,:))) - kcatM(11).*FadM.*y(393,:)./(Km_FadM_keto_acyl_CoA(11)+y(393,:));%20:1

%FadM forming methyl-ketones from keto-acyl-CoA (and keto-acyl-ACP, but
%this is turned off in the model by setting that kcat equal to zero)
dy(394,:) = kcatM(1).*FadM.*y(381,:)./(Km_FadM_keto_acyl_CoA(1)+y(381,:)) + kcatM_ACP(1).*FadM.*y(17,:)./(Km_FadM_keto_acyl_CoA(1)+y(17,:));%6
dy(395,:) = kcatM(2).*FadM.*y(382,:)./(Km_FadM_keto_acyl_CoA(2)+y(382,:)) + kcatM_ACP(2).*FadM.*y(22,:)./(Km_FadM_keto_acyl_CoA(2)+y(22,:));%8
dy(396,:) = kcatM(3).*FadM.*y(383,:)./(Km_FadM_keto_acyl_CoA(3)+y(383,:)) + kcatM_ACP(3).*FadM.*y(27,:)./(Km_FadM_keto_acyl_CoA(3)+y(27,:));%10
dy(397,:) = kcatM(4).*FadM.*y(384,:)./(Km_FadM_keto_acyl_CoA(4)+y(384,:)) + kcatM_ACP(4).*FadM.*y(33,:)./(Km_FadM_keto_acyl_CoA(4)+y(33,:));%12
dy(398,:) = kcatM(5).*FadM.*y(385,:)./(Km_FadM_keto_acyl_CoA(5)+y(385,:)) + kcatM_ACP(5).*FadM.*y(43,:)./(Km_FadM_keto_acyl_CoA(5)+y(43,:));%14
dy(399,:) = kcatM(6).*FadM.*y(386,:)./(Km_FadM_keto_acyl_CoA(6)+y(386,:)) + kcatM_ACP(6).*FadM.*y(53,:)./(Km_FadM_keto_acyl_CoA(6)+y(53,:));%16
dy(400,:) = kcatM(7).*FadM.*y(387,:)./(Km_FadM_keto_acyl_CoA(7)+y(387,:)) + kcatM_ACP(7).*FadM.*y(63,:)./(Km_FadM_keto_acyl_CoA(7)+y(63,:));%18
dy(401,:) = kcatM(8).*FadM.*y(388,:)./(Km_FadM_keto_acyl_CoA(8)+y(388,:)) + kcatM_ACP(8).*FadM.*y(38,:)./(Km_FadM_keto_acyl_CoA(8)+y(38,:));%12_unsat
dy(402,:) = kcatM(9).*FadM.*y(389,:)./(Km_FadM_keto_acyl_CoA(9)+y(389,:)) + kcatM_ACP(9).*FadM.*y(48,:)./(Km_FadM_keto_acyl_CoA(9)+y(48,:));%14_unsat
dy(403,:) = kcatM(10).*FadM.*y(390,:)./(Km_FadM_keto_acyl_CoA(10)+y(390,:)) + kcatM_ACP(10).*FadM.*y(58,:)./(Km_FadM_keto_acyl_CoA(10)+y(58,:));%16_unsat
dy(404,:) = kcatM(11).*FadM.*y(391,:)./(Km_FadM_keto_acyl_CoA(11)+y(391,:)) + kcatM_ACP(11).*FadM.*y(68,:)./(Km_FadM_keto_acyl_CoA(11)+y(68,:));%18_unsat
dy(405,:) = kcatM(7).*FadM.*y(392,:)./(Km_FadM_keto_acyl_CoA(7)+y(392,:));%19
dy(406,:) = kcatM(11).*FadM.*y(393,:)./(Km_FadM_keto_acyl_CoA(11)+y(393,:));%19_unsat
