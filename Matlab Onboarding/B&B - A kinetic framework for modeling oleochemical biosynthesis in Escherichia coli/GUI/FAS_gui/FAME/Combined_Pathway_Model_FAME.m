function dy = Combined_Pathway_Model_FAME(t,y,param_list,opt_struct,p_vec_k)
...
%FAME production kinetic constants
%DmJHAMT
kcatC = p_vec_k(2).*[0.0025*p_vec_k(1)	0.001032495	0.000765199	0.0025	0.001640461	0.000765199];  %DOI: 10.1038/srep24239
Km_SAM = 59;
Km_FA = [59 59 59 59 59 59]; % DOI: 10.1038/srep24239

%cofactor concentrations (uM)
SAM = opt_struct.aux_enz(1);

%Enzyme concentrations (uM)
Dm = opt_struct.aux_enz(2);
...
dy(26,:) = kcat7(3).*y(131,:);
dy(31,:) = kcat7(4).*y(146,:);
dy(37,:) = kcat7(5).*y(161,:) - kcatC(1).*Dm.*SAM.*y(37,:)./((Km_SAM+SAM).*(Km_FA(1)+y(37,:)));
dy(47,:) = kcat7(6).*y(176,:) - kcatC(2).*Dm.*SAM.*y(47,:)./((Km_SAM+SAM).*(Km_FA(2)+y(47,:)));
dy(57,:) = kcat7(7).*y(191,:) - kcatC(3).*Dm.*SAM.*y(57,:)./((Km_SAM+SAM).*(Km_FA(3)+y(57,:)));
dy(67,:) = kcat7(8).*y(206,:);
dy(77,:) = kcat7(9).*y(221,:);

dy(42,:) = kcat7(5).*y(230,:) - kcatC(4).*Dm.*SAM.*y(42,:)./((Km_SAM+SAM).*(Km_FA(4)+y(42,:)));
dy(52,:) = kcat7(6).*y(245,:) - kcatC(5).*Dm.*SAM.*y(52,:)./((Km_SAM+SAM).*(Km_FA(5)+y(52,:)));
dy(62,:) = kcat7(7).*y(260,:) - kcatC(6).*Dm.*SAM.*y(62,:)./((Km_SAM+SAM).*(Km_FA(6)+y(62,:)));
dy(72,:) = kcat7(8).*y(275,:);
dy(82,:) = kcat7(9).*y(290,:);
...
%Production of FAMEs
dy(337,:) = kcatC(1).*Dm.*SAM.*y(37,:)./((Km_SAM+SAM).*(Km_FA(1)+y(37,:)));%12
dy(338,:) = kcatC(2).*Dm.*SAM.*y(47,:)./((Km_SAM+SAM).*(Km_FA(2)+y(47,:)));%14
dy(339,:) = kcatC(3).*Dm.*SAM.*y(57,:)./((Km_SAM+SAM).*(Km_FA(3)+y(57,:)));%16

dy(340,:) = kcatC(4).*Dm.*SAM.*y(42,:)./((Km_SAM+SAM).*(Km_FA(4)+y(42,:)));%12_unsat
dy(341,:) = kcatC(5).*Dm.*SAM.*y(52,:)./((Km_SAM+SAM).*(Km_FA(5)+y(52,:)));%14_unsat
dy(342,:) = kcatC(6).*Dm.*SAM.*y(62,:)./((Km_SAM+SAM).*(Km_FA(6)+y(62,:)));%16_unsat
