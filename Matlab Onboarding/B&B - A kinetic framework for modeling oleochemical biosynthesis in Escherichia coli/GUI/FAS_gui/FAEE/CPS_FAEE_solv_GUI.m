%FAS solver Function
%Solves the system of ordindary differential equations consisting of the kinetic equations of the fatty acid pathway 
%   Input:
...
function results = CPS_FAEE_solv_GUI(user)
...
parameterized_Combined_Pathway_Model = @(t,c) Combined_Pathway_Model_FAEE(t,c,param_struct,opt_struct,p_vec_k);
...
results.acyl_CoA_time = sum(C(:,348:358),2);
results.FAEE_time = sum(FAEE_raw,2);
results.time = T;
