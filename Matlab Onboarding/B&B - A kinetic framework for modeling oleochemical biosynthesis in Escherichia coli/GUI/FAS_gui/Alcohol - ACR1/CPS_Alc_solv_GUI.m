...
elseif strcmp(TE,'BTE')
    p_vec_k = [0.802490234375000  0.680651352240870	132.988017008123 0.146865102220244]; %% BTE
else
    p_vec_k = [10  0.680651352240870	132.988017008123 0.146865102220244]; %% CpFatB
end 
...
parameterized_Combined_Pathway_Model = @(t,c) Combined_Pathway_Model_Alc(t,c,param_struct,opt_struct,p_vec_k);
...
weight_vec = [6 8 10 12 14 16 18 12 14 16 18]./16;

FA_raw = C(:,370:380);
FA_alc = C(end,370:380);

tot_prod = sum(FA_alc);
palm_equiv = sum(FA_alc.*weight_vec);
frac_alc = FA_alc/tot_prod;
avg_CL = sum(frac_alc.*weight_vec*16);
unsat_frac = sum(FA_alc(8:11))/tot_prod;
...
results.aCoA_time = sum(C(:,348:358),2);
results.aldehyde_time = sum(C(:,359:369),2);
