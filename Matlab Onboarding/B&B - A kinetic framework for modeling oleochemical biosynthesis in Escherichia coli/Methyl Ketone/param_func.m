...
scaling_factor_k8_7f = opt_struct.('scaling_factor_k8_7f');
scaling_factor_k10_4f = opt_struct.('scaling_factor_k10_4f');
...
for i = 1:length(param_names)
    ...
    elseif ismember(param_names{i},{'k2_1f','k2_1r','k2_3f','k2_3r','k3_1f','k3_1r','k3_3f','k3_3r',...
        ...
        elseif ismember(param_names{i},{'k8_7f'})
            param_struct.(param_names{i}) = param_table{{param_names{i}},:}*scaling_factor_k8_7f;
        elseif ismember(param_names{i},{'k10_4f'})
            param_struct.(param_names{i}) = param_table{{param_names{i}},:}*scaling_factor_k10_4f;
        else
...
elseif strcmp(name,'k7_1f') || strcmp(name,'kcat7')
    ...
    if strcmp(name,'k7_1f')
        if strcmp(TesA_source,'Pf') || strcmp(TesA_source,'TesA')
...
%                 end
        ...
        elseif strcmp(TesA_source,'CpFatB') %CpFatB1
            kd_est = [440,273.8140574,0.98954855,13.77921031,440,440,440,440,440];
            for i = 1:num_elong_steps
                param(i) = param_struct.('k7_1r')/kd_est(i);%for all chain lengths
            end
        elseif strcmp(TesA_source,'BTE') %UcFatB2
            kd_est = [440,440,440,273.8140574,3.520487177,13.77921031,440,440,440];
            for i = 1:num_elong_steps
                param(i) = param_struct.('k7_1r')/kd_est(i);%for all chain lengths
            end
        ...
        elseif isvector(TesA_source) %Same as Pf source of TesA because we are only changing the kcat with the optimizer.
            kd_12 = exp(lin_slope*(12) + lin_int);%kd estimated from linear free energy relationship for chain lengths 12-20
            ratio_val = kd_12/(0.519*14.79);%ratio used to match kd at chain length 12
            kd_est = (ratio_val).*[473 293.9 52.986 14.79];%Kds for 4,6 and 8-12, estimated Kd is scaled to match linear free energy values
            for i = 1:num_elong_steps
    ...
    elseif strcmp(name,'kcat7')
        ...
        elseif strcmp(TesA_source,'CpFatB')
            kcat_scaling = [0,0.104538367,2.767215676,0.032450024,0,0,0,0,0];
            for i = 1:num_elong_steps
                param(i) = kcat_scaling(i);
            end
        elseif strcmp(TesA_source,'BTE')
            kcat_scaling = [0,0,0,0.104538367,1.37285756,0.032450024,0,0,0];
            for i = 1:num_elong_steps
                param(i) = kcat_scaling(i);
            end
        elseif strcmp(TesA_source,'Non-native')
        ...
        elseif isvector(TesA_source)
            kcat7 = TesA_source; %In this case, TesA sourse is a kcat vector, not a string.
            param = kcat7;
        end
    end
...
elseif ismember(name,{'kcat5','kcat8','kcat9','kcat10'})
    ...
    elseif strcmp(name,'kcat8')
        ...
        kcat_scaling_fabF = [0.914,0.914,0.901,1,0.9,0.289,0.0222,0,0];
        for i = 1:num_elong_steps
    ...
    elseif strcmp(name,'kcat10')
        ...
        kcat_scaling_fabB = [0.855,0.855,0.975,0.967,1,0.125,0.0208,0,0];
        for i = 1:num_elong_steps
