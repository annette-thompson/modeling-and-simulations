...
function [Y, W, Z] = model_evaluation(fun_test,X,varargin)
%
...
elseif (NumExtraArgIn<=0)&&(NumExtraArgOut>0)
    for j=1:N
        %added this j to see if i could recover all the outputs. 
        %[Y(j,:),varargout{j,1:NumExtraArgOut}] = feval(fun_test,X(j,:)) ;
        [Y(j,:), W(j,:), Z(j,:)] = feval(fun_test,X(j,:));
    end
else
    for j=1:N
