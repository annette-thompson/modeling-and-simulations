...
%       val_data: name of validatation file written as a string, note 
%           the validation file must be csv with only two columns, one with 
...
pp = spline(t_vec,mod_res); %makes a line between the points using cubic polynomials

yy = ppval(pp,xx); %gets the values using the line and the data points from the file

s = sum((yy-conc_data).^2); %SSE comparing line valuess and file values
