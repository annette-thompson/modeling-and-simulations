...
% mu= mean, sigma = stdev
% AcCoA OcCoA ACP NADPH NADH MalCoA FabD FabH FabG FabZ FabI TesA FabF FabA FabB
...
% Scaling for FabH
EC_kcat3_scaling = [1,0,0,0,0,0,0,0,0]; % unfit
PP_H1_kcat3_scaling = [5.6367,0,0,0,0,0,0,0,0]; % Fit 2A
PP_H2_kcat3_scaling = [0,0,0,3.7639,0,0,0,0,0]; % Fit fixed 2B
...
stdev_rel_rate = std(rel_rate);
mean_NAD_rate = mean(NAD_rate);
stdev_NAD_rate = std(NAD_rate);
...
summary_data = table(mean_rel_rate, stdev_rel_rate, mean_NAD_rate, stdev_NAD_rate,...
    'VariableNames', {'MeanRelRate', 'StdevRelRate', 'MeanNADRate', 'StdevNADRate'});

writetable(output_data, 'Model_error_output.csv');
writetable(summary_data, 'Model_error_summary.csv');