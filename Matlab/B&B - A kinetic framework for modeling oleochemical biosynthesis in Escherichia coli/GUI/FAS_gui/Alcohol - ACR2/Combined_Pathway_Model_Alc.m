...
%ACR2
%if optimizing FadD conc.
% kcatACR2 = 115e-9.*10^3.*50000./60.*[0.09  0.09  0.09  0.26  0.60  1  0.69  0.26  0.60  0.99  0.78]; %doi.org/10.1021/bi2008646

%if optimizing C18 kcat instead of FadD conc.
kcatACR2 = 115e-9.*10^3.*50000./60.*[0.09  0.09  0.09  0.26  0.60  1  0.69*p_vec_k(3)  0.26  0.60  0.99  0.78]; %doi.org/10.1021/bi2008646
Km_ACR2 = 4; %doi.org/10.1021/bi2008646
n_ACR2 = 1; %doi.org/10.1021/bi2008646
...
dy(348,:) = kcatD2.*FadD.*y(9,:).*y(337,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(1)+y(337,:))) - kcatACR2(1).*ACR2.*y(348,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(348,:).^n_ACR2);%6
dy(349,:) = kcatD2.*FadD.*y(9,:).*y(338,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(2)+y(338,:))) - kcatACR2(2).*ACR2.*y(349,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(349,:).^n_ACR2);%8
dy(350,:) = kcatD2.*FadD.*y(9,:).*y(339,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(3)+y(339,:))) - kcatACR2(3).*ACR2.*y(350,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(350,:).^n_ACR2);%10
dy(351,:) = kcatD2.*FadD.*y(9,:).*y(340,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(4)+y(340,:))) - kcatACR2(4).*ACR2.*y(351,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(351,:).^n_ACR2);%12
dy(352,:) = kcatD2.*FadD.*y(9,:).*y(341,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(5)+y(341,:))) - kcatACR2(5).*ACR2.*y(352,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(352,:).^n_ACR2);%14
dy(353,:) = kcatD2.*FadD.*y(9,:).*y(342,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(6)+y(342,:))) - kcatACR2(6).*ACR2.*y(353,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(353,:).^n_ACR2);%16
dy(354,:) = kcatD2.*FadD.*y(9,:).*y(343,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(7)+y(343,:))) - kcatACR2(7).*ACR2.*y(354,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(354,:).^n_ACR2);%18
dy(355,:) = kcatD2.*FadD.*y(9,:).*y(344,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(8)+y(344,:))) - kcatACR2(8).*ACR2.*y(355,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(355,:).^n_ACR2);%12_unsat
dy(356,:) = kcatD2.*FadD.*y(9,:).*y(345,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(9)+y(345,:))) - kcatACR2(9).*ACR2.*y(356,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(356,:).^n_ACR2);%14_unsat
dy(357,:) = kcatD2.*FadD.*y(9,:).*y(346,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(10)+y(346,:)))- kcatACR2(10).*ACR2.*y(357,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(357,:).^n_ACR2);%16_unsat
dy(358,:) = kcatD2.*FadD.*y(9,:).*y(347,:)./((Km_CoA+y(9,:)).*(Km_FA_AMP(11)+y(347,:)))- kcatACR2(11).*ACR2.*y(358,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(358,:).^n_ACR2);%18_unsat

%Fatty Alcohol Production
dy(359,:) = kcatACR2(1).*ACR2.*y(348,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(348,:).^n_ACR2);%6
dy(360,:) = kcatACR2(2).*ACR2.*y(349,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(349,:).^n_ACR2);%8
dy(361,:) = kcatACR2(3).*ACR2.*y(350,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(350,:).^n_ACR2);%10
dy(362,:) = kcatACR2(4).*ACR2.*y(351,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(351,:).^n_ACR2);%12
dy(363,:) = kcatACR2(5).*ACR2.*y(352,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(352,:).^n_ACR2);%14
dy(364,:) = kcatACR2(6).*ACR2.*y(353,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(353,:).^n_ACR2);%16
dy(365,:) = kcatACR2(7).*ACR2.*y(354,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(354,:).^n_ACR2);%18
dy(366,:) = kcatACR2(8).*ACR2.*y(355,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(355,:).^n_ACR2);%12_unsat
dy(367,:) = kcatACR2(9).*ACR2.*y(356,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(356,:).^n_ACR2);%14_unsat
dy(368,:) = kcatACR2(10).*ACR2.*y(357,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(357,:).^n_ACR2);%16_unsat
dy(369,:) = kcatACR2(11).*ACR2.*y(358,:).^n_ACR2./(Km_ACR2.^n_ACR2 + y(358,:).^n_ACR2);%18_unsat
