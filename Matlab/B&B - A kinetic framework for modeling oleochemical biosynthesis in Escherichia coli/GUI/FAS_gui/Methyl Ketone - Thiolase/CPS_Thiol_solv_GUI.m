...
function results = CPS_Thiol_solv_GUI(user)
...
opt_struct = struct(field5,val5,field6,val6,field10,val10,field11,val11,field12,val12,field13,val13,field14,val14,field15,val15,...
    ...
    field26,val26,field27,val27,field28,val28,field29,val29,field30,val30,field31,val31,field32, val32,field33,val33);
...
ket_raw = C(:,394:406);
FA_ratio = [C(end,37)/C(end,395) C(end,47)/C(end,396) C(end,52)/C(end,400) C(end,57)/C(end,397) C(end,62)/C(end,401)];

results.acyl_ACP_dist = 0;
results.aldehyde_dist = 0;
results.MK_dist = [ket_amt(1:7) ket_amt(end-1) ket_amt(8:11) ket_amt(end)];

results.acyl_ACP_time = C(:,15)+C(:,20)+C(:,25)+C(:,30)+C(:,36)+C(:,46)+C(:,56)+C(:,66)+C(:,76)+C(:,41)+C(:,51)+C(:,61)+C(:,71)+C(:,81);
results.FFA_time = sum(F_raw_all,2);
results.acyl_CoA_time = sum(C(:,337:348),2);
results.MK_time = sum(ket_raw,2);
results.time = T;
