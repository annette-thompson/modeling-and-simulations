%FAS solver Function
%Solves the system of ordindary differential equations consisting of the kinetic equations of the fatty alcohol pathway 
%   Input:
...
function [results] = CPS_Alc_solv_GUI(user)
...
%Generates model function with all constants from the parameterization
parameterized_Combined_Pathway_Model = @(t,c) Combined_Pathway_Model_Alc(t,c,param_struct,opt_struct);
...
F_weight_vec = [4,6,8,10,12,12,14,14,16,16,18,18,20,20]/16;
count = 1;
for ind = 1:length(labels)
    label_val = char(labels(ind));
    first_char = label_val(1);
    if first_char == char('F')
        F_saved(count) = F_weight_vec(count)*(C(end,ind));
        F_raw_all(:,count) = C(:,ind);
        F_weighted = F_weight_vec(count)*(C(:,ind)) + F_weighted;
        count = count + 1;
    end
end
...
weight_vec = [4 6 8 10 12 14 16 18 12 14 16 18]./16;

FA_raw = C(:,349:360);
FA_alc = C(end,349:360);

tot_prod = sum(FA_alc);
palm_equiv = sum(FA_alc.*weight_vec);
frac_alc = FA_alc/tot_prod;
avg_CL = sum(frac_alc.*weight_vec*16);
unsat_frac = sum(FA_alc(9:12))/tot_prod;

results.acyl_ACP_dist = 0;
results.aldehyde_dist = 0;
results.alcohol_dist = FA_alc;
...
results.aldehyde_time = sum(C(:,337:348),2);
results.alcohol_time = sum(FA_raw,2);
results.time = T;
